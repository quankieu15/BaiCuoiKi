<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('partner.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="category_id" name="category_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm" required>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" data-slug="{{ Str::slug($cat->name) }}" {{ $service->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input id="title" name="title" type="text" class="block mt-1 w-full" value="{{ $service->title }}" required />
                    </div>

                    <div>
                        <x-input-label for="price" :value="__('Giá bán (đ)')" />
                        <x-text-input id="price" name="price" type="number" class="block mt-1 w-full" value="{{ $service->price }}" required />
                    </div>

                    <div>
                        <x-input-label for="tour_location_input" :value="__('Địa điểm')" />
                        <x-text-input id="tour_location_input" name="location" type="text" class="block mt-1 w-full" value="{{ $service->location }}" required />
                    </div>
                            ...
                            @foreach($hotels as $hotel)
                                <label class="hotel-item flex items-start gap-3 p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer transition-colors text-gray-800 dark:text-gray-200">
                                    <input type="checkbox" name="hotel_ids[]" value="{{ $hotel->id }}" 
                                           {{ $service->hotels->contains($hotel->id) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm" required>{{ $service->description }}</textarea>
                    </div>

                    <div>
                        <x-input-label for="image" :value="__('Thay đổi hình ảnh (Bỏ trống nếu giữ ảnh cũ)')" />
                        @if($service->image)
                            <img src="{{ asset($service->image) }}" class="w-32 h-20 object-cover rounded my-2">
                        @endif
                        <input id="image" name="image" type="file" class="block mt-1 w-full text-slate-500 dark:text-slate-400" />
                    </div>

                    <div class="flex justify-end space-x-2 pt-4">
                        <a href="{{ route('partner.services.index') }}" class="bg-gray-500 text-white font-bold py-2 px-4 rounded text-sm">Quay lại</a>
                        <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded text-sm">Cập nhật ngay</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // 1. Hàm kiểm tra ẩn/hiện hộp khách sạn dựa theo loại hình dịch vụ
            ...
            // 2. Hàm lọc danh sách khách sạn theo chuỗi ký tự ở ô Địa điểm
            ...
            // Kích hoạt chạy ngay lần đầu tiên tải trang để thiết lập trạng thái ban đầu
