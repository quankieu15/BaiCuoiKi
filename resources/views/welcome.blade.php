...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-30">
    <div class="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.06)] border border-slate-100">
        ...
        <div class="flex flex-wrap items-center justify-center gap-3 md:gap-4 mb-8 pb-6 border-b border-slate-100">
            ...
            <a href="/?type=domestic_tour"
               class="flex items-center gap-2 px-5 py-3 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300 {{ request('type') == 'domestic_tour' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 scale-105' : 'bg-slate-50 text-slate-600 hover:bg-slate-100' }}">
                🏝️ Tour Trong Nước
            </a>

            <a href="/?type=international_tour"
               class="flex items-center gap-2 px-5 py-3 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300 {{ request('type') == 'international_tour' ? 'bg-rose-600 text-white shadow-lg shadow-rose-600/30 scale-105' : 'bg-slate-50 text-slate-600 hover:bg-slate-100' }}">
                ✈️ Tour Nước Ngoài
            </a>
        ...
        <form action="/" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            {{-- CHỮA LỖI: Đồng bộ hóa hidden field đi theo biến type thống nhất --}}
            @if(request('type'))
            ...
            <div class="text-left px-4 flex flex-col justify-center bg-[#fef9c3] hover:bg-[#fef08a] p-3.5 rounded-2xl transition border border-yellow-200">
                ...
                <select name="type" class="w-full bg-transparent focus:outline-none font-bold text-yellow-950 py-0.5 text-sm cursor-pointer appearance-none">
                    ...
                    <option value="domestic_tour" {{ request('type') == 'domestic_tour' ? 'selected' : '' }}>✈️ Tour trong nước</option>
                    <option value="international_tour" {{ request('type') == 'international_tour' ? 'selected' : '' }}>✈️ Tour quốc tế</option>
        ...
        {{-- SỬA CHUẨN: Đang bật bộ lọc hiển thị Tiếng Việt trực tiếp dựa theo biến `type` --}}
        @if(request('location') || request('type'))
            <div class="flex flex-wrap items-center justify-between gap-3 mt-6 text-xs text-slate-500 font-medium bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @if(request('type')) 
                        <span class="bg-white px-3 py-1 rounded-lg border border-slate-200">Loại: 
            ...
            <div class="bg-white rounded-[24px] shadow-[0_10px_40px_rgba(14,165,233,0.08)] hover:shadow-[0_25px_60px_rgba(14,165,233,0.18)] border border-slate-100 overflow-hidden transition-all duration-300 flex flex-col justify-between group transform hover:-translate-y-1">
                <div>
                    ...
                    <div class="p-6 space-y-4 bg-gradient-to-b from-white to-sky-50/40">
                        ...
                        {{-- TIÊU ĐỀ --}}
                        <h3 class="font-extrabold text-base md:text-lg text-slate-800 line-clamp-2 leading-snug group-hover:text-sky-600 transition duration-300 h-[56px]" title="{{ $service->title }}">
                            {{ $service->title }}
                        </h3>

                        {{-- ĐÁNH GIÁ ĐÃ ĐƯỢC THAY BẰNG CODE BẢO VỆ CHỐNG LỖI --}}
                        <div class="flex items-center gap-2 text-xs font-bold text-left">
                            @php
                                $totalReviews = 0;
                                $avgRating = 0;

                                // Kiểm tra an toàn xem có tồn tại quan hệ reviews trong biến $service không
                                if (isset($service->reviews) && is_iterable($service->reviews)) {
                                 $approvedReviews = collect($service->reviews)->where('is_visible', 1);
                                    $totalReviews = $approvedReviews->count();
                                    $avgRating = $totalReviews > 0 ? round($approvedReviews->avg('rating'), 1) : 0;
                                }
                                
                                $starLoop = (int) round($avgRating);
                            @endphp
                            
                            @if($totalReviews > 0)
                                <div class="flex items-center bg-gradient-to-r from-yellow-50 to-amber-100/70 text-amber-500 px-2.5 py-1 rounded-xl border border-amber-200/60 shadow-sm gap-0.5">
                                    @for ($i = 1; $i <= $starLoop; $i++)
                                        ⭐
                                    @endfor
                                    
                                    <span class="ml-1 font-black text-slate-700 text-[11px]">
                            ...
                            @else
                                <span class="text-slate-400 font-medium italic bg-slate-100 px-3 py-1 rounded-xl border border-slate-200">
                                    Chưa có đánh giá
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-2 pt-1 flex-wrap">
                            <span class="bg-sky-100 text-sky-700 text-[10px] px-3 py-1 rounded-full font-bold tracking-wide">
                                🌊 Du lịch biển
                            </span>
                            <span class="bg-cyan-100 text-cyan-700 text-[10px] px-3 py-1 rounded-full font-bold tracking-wide">
                                ✈️ Premium Tour
                            </span>
                        </div>
...
<footer class="bg-gradient-to-br from-cyan-400 via-sky-500 to-blue-500 text-white pt-16 pb-8 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 pb-12 border-b border-white/20">
            <div class="space-y-5">
                <div class="text-3xl font-black tracking-wider">
                    <span class="text-white drop-shadow">HKT</span>
                    <span class="text-yellow-200 border-b-4 border-yellow-200 pb-1">TRAVEL</span>
                </div>
                <p class="text-sm leading-relaxed text-white/90">
                    HKT TRAVEL mang đến trải nghiệm du lịch hiện đại, trẻ trung và tiện lợi với hệ thống đặt tour, khách sạn và nghỉ dưỡng thông minh.
                </p>
                <div class="flex gap-3 pt-2">
                    <div class="w-11 h-11 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center hover:bg-white hover:text-sky-600 transition-all duration-300 cursor-pointer text-lg shadow-lg">🌍</div>
                    <div class="w-11 h-11 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center hover:bg-white hover:text-sky-600 transition-all duration-300 cursor-pointer text-lg shadow-lg">✈️</div>
                    <div class="w-11 h-11 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center hover:bg-white hover:text-sky-600 transition-all duration-300 cursor-pointer text-lg shadow-lg">🏖️</div>
                </div>
            </div>

            <div>
                <h4 class="text-white font-extrabold uppercase tracking-widest text-sm mb-5">Dịch vụ</h4>
                <ul class="space-y-3 text-sm text-white/90">
                    <li class="hover:text-yellow-200 transition cursor-pointer">✈️ Tour trong nước</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🌏 Tour quốc tế</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🏨 Resort & khách sạn</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🚗 Thuê xe du lịch</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🎟️ Vé tham quan</li>
                </ul>
            </div>

            <div>
                <h4 class="text-white font-extrabold uppercase tracking-widest text-sm mb-5">Thông tin</h4>
                <ul class="space-y-3 text-sm text-white/90">
                    <li class="hover:text-yellow-200 transition cursor-pointer">📰 Tin tức du lịch</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🔒 Chính sách bảo mật</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">💳 Thanh toán online</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">📑 Điều khoản dịch vụ</li>
                    <li class="hover:text-yellow-200 transition cursor-pointer">🤝 Hợp tác doanh nghiệp</li>
                </ul>
            </div>

            <div>
                <h4 class="text-white font-extrabold uppercase tracking-widest text-sm mb-5">Liên hệ</h4>
                <div class="space-y-4 text-sm text-white/90 leading-relaxed">
                    <p>📍 Phenikaa Tower, Hà Đông, Hà Nội</p>
                    <p>📞 Hotline: 1900 6868</p>
                    <p>📧 support@hkttravel.vn</p>
        ...
        <div class="pt-8 text-center space-y-3">
            <p class="text-sm italic text-white/90">“Explore The World — Trải nghiệm hành trình tuyệt vời cùng HKT TRAVEL.”</p>
            <p class="text-[11px] uppercase tracking-[3px] text-white/70 font-bold">© 2026 HKT TRAVEL — PHENIKAA UNIVERSITY</p>
        </div>
    </div>
</footer>
</body>
